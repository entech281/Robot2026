...
/**
 * The {@code HMAccelerometer} class contains fields and methods pertaining to the function of the
 * accelerometer.
 */
public class HMAccelerometer {
  ...
  public double getAccurateTilt() {
    ...
    try {
      return Math.toDegrees(
          Math.atan(Math.sqrt(Math.pow(getAccelX(), 2) + Math.pow(getAccelY(), 2)) / getAccelZ()));
  ...
  public double getAccuratePitch() {
    ...
    try {
      return Math.toDegrees(
          Math.atan(getAccelX() / Math.sqrt(Math.pow(getAccelY(), 2) + Math.pow(getAccelZ(), 2))));
    } catch (ArithmeticException e) {
  ...
  public double getAccurateRoll() {
    ...
    try {
      return Math.toDegrees(
          Math.atan(getAccelY() / Math.sqrt(Math.pow(getAccelX(), 2) + Math.pow(getAccelZ(), 2))));
    } catch (ArithmeticException e) {
      return 90; // if denominator is zero, we are dead
    }
  }
